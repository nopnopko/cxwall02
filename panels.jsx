/* ============ Panels & UI pieces (Ant Design) ============ */
const { useState, useEffect, useRef } = React;
const { Dropdown, Button, Avatar, Tooltip, Statistic, Progress: AntProgress, Collapse } = antd;

/* ---------- HEADER ---------- */
function Header({ t, lang, setLang, theme, setTheme, now, openSettings, toggleFullscreen }) {
  ...
  const time = window.DASH.fmtClock(now.getHours()*3600 + now.getMinutes()*60 + now.getSeconds());
  ...
  const themeItems = themes.map(([k,label,sw]) => ({
    ...
    label: (<span style={{display:"flex",alignItems:"center",gap:10,minWidth:160,fontWeight:theme===k?700:500}}>
      <span className="dd-sw" style={{background:sw}}></span><span>{label}</span>{theme===k && checkmark}</span>),
  }));
  const profileItems = [
  ...
  return (
    <header className="hd">
      <div className="hd-brand">
        ...
        <div className="hd-title" data-cc-editable>{t.appTitle}<small>{t.appSub}</small></div>
      </div>
      <div className="hd-spacer"></div>

      <div className="hd-live"><span className="hd-dot"></span>{t.live}</div>
      ...
      <div className="hd-grp">
        <Dropdown trigger={["click"]} placement="bottomRight"
          menu={{ items: langItems, onClick: ({key}) => setLang(key), selectedKeys:[lang] }}>
          <Tooltip title={t.language}><Button className="hd-icobtn" type="text" icon={Ico.globe(18)} /></Tooltip>
        </Dropdown>

        <Dropdown trigger={["click"]} placement="bottomRight"
          menu={{ items: themeItems, onClick: ({key}) => setTheme(key), selectedKeys:[theme] }}>
        ...
        <Dropdown trigger={["click"]} placement="bottomRight"
          menu={{ items: profileItems, onClick: ({key}) => { if(key==="settings") openSettings(); } }}>
      ...
      {cards.map(c => (
        <div key={c.k} className={"kpi "+c.cls}>
          ...
          <span className="kpi-val">
            <Statistic value={c.val} valueStyle={{fontSize:56,fontWeight:800,lineHeight:1,color:c.color}} />
          </span>
        </div>
      ))}
    </div>
...
function ChannelRow({ t, channels, order, onReorder }) {
  ...
  return (
    <div className="chrow" style={{ gridTemplateColumns: `repeat(${visible.length},1fr)` }}>
      {visible.map(key => {
        ...
        return (
          <div key={key}
            className={"chcard"+(drag===key?" dragging":"")+(over===key?" dragover":"")}
            draggable
...
/* ---------- QUEUE PANEL ---------- */
function QueuePanel({ t, channels, order, queueChannels, flat }) {
  const byKey = Object.fromEntries(channels.map(c => [c.key, c]));
    ...
    <div className="avg-stack">
      <div className="avg-card avg-talk">
        ...
        <div className="v"><span style={{color:"var(--talk)",display:"flex"}}>{Ico.clock(22)}</span>
          <Statistic value={window.DASH.fmtClock(avg.talk)} valueStyle={monoStyle("var(--talk)")} /></div>
      </div>
      <div className="avg-card avg-wait">
        ...
        <div className="v"><span style={{color:"var(--avail)",display:"flex"}}>{Ico.hourglass(20)}</span>
          <Statistic value={window.DASH.fmtClock(avg.wait)} valueStyle={monoStyle("var(--avail)")} /></div>
      </div>
    </div>
  );
}
...
/* ---------- IVR PANEL ---------- */
function IvrPanel({ t, ivr, maxIvr }) {
          ...
          {items.map((it,i)=>(
            <div className="ivr-row" key={i}>
              ...
              <div className="ivr-bar-wrap">
                <AntProgress percent={(it.val/maxAxis)*100} showInfo={false} strokeColor={talk} />
...
Object.assign(window, { Header, KpiBar, ChannelRow, QueuePanel, AvgCards, IvrPanel });
