/* ============ Icons: brand channel glyphs (custom) + AntD UI icons ============ */
/* Channel logos are brand-accurate SVGs (AntD has no brand marks).
   Every generic UI icon is rendered from @ant-design/icons (window.icons). */

const AntIco = window.icons || {};

// ---- channel glyphs (brand-accurate, white on colored app-icon badge) ----
function ChannelGlyph({ ch, size = 18 }) {
...
function ChannelBadge({ ch, color, size = 32, radius = 9, glyph = 18 }) {
  return (
    <span style={{ width: size, height: size, borderRadius: radius, background: color, color: "#fff" }}
      className="ch-badge" >
...
// ---- UI icons: thin wrappers over @ant-design/icons so size stays a number ----
const W = (Comp) => (s = 18) => Comp ? <Comp style={{ fontSize: s, lineHeight: 0, display: "flex" }} /> : null;
const Ico = {
  headset:   W(AntIco.CustomerServiceOutlined),
  talk:      W(AntIco.PhoneFilled),
  user:      W(AntIco.UserOutlined),
  userOff:   W(AntIco.UserDeleteOutlined),
  clock:     W(AntIco.ClockCircleOutlined),
  calendar:  W(AntIco.CalendarOutlined),
  gear:      W(AntIco.SettingOutlined),
  globe:     W(AntIco.GlobalOutlined),
  palette:   W(AntIco.BgColorsOutlined),
  expand:    W(AntIco.FullscreenOutlined),
  refresh:   W(AntIco.ReloadOutlined),
  grip:      W(AntIco.HolderOutlined),
  chevron:   W(AntIco.DownOutlined),
  check:     W(AntIco.CheckOutlined),
  queue:     W(AntIco.TeamOutlined),
  x:         W(AntIco.CloseOutlined),
  hourglass: W(AntIco.HourglassOutlined),
};

Object.assign(window, { ChannelGlyph, ChannelBadge, Ico });
